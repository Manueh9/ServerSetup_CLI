...
class TestInstallUfw:
    def test_skips_if_already_installed(self, mocker, mock_run_command):
        mocker.patch("modules.ufw.is_ufw_installed", return_value=True)
        mock = mock_run_command("modules.ufw")
        install_ufw()
        mock.assert_not_called()

    def test_installs_if_not_present(self, mocker, mock_run_command):
        mocker.patch("modules.ufw.is_ufw_installed", return_value=False)
        mock = mock_run_command("modules.ufw")
        install_ufw()
...
class TestEnableUfw:
    def test_skips_if_already_active(self, mocker, mock_run_command):
        mocker.patch("modules.ufw.is_ufw_active", return_value=True)
        mock = mock_run_command("modules.ufw")
        enable_ufw()
        mock.assert_not_called()

    def test_enables_with_force_flag_if_inactive(self, mocker, mock_run_command):
        mocker.patch("modules.ufw.is_ufw_active", return_value=False)
        mock = mock_run_command("modules.ufw")
        enable_ufw()
