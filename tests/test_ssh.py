import pytest
from modules.ssh import change_ssh_port, _verify_config_port
...
class TestChangeSSHPort:
    ...
    def test_replaces_commented_port_line(self, tmp_path, monkeypatch, mocker):
        ...
        monkeypatch.setattr("modules.ssh.SSH_CONFIG_FILE", str(config_file))
    ...
    def test_permission_error_exits(self, monkeypatch):
        monkeypatch.setattr("modules.ssh.SSH_CONFIG_FILE", "/root/protected_file_that_does_not_exist")
        with pytest.raises(SystemExit):
            change_ssh_port(2222)
